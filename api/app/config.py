...
class Settings(BaseSettings):
    ...
    admin_username: str = Field(default="admin", validation_alias=AliasChoices("ADMIN_USERNAME", "API_ADMIN_USERNAME"))
    admin_password_hash: str = Field(default="", alias="ADMIN_PASSWORD_HASH")
    admin_password: str = Field(default="", validation_alias=AliasChoices("ADMIN_PASSWORD", "API_ADMIN_PASSWORD"))
    api_auth_disabled: bool = Field(default=False, alias="API_AUTH_DISABLED")

    jwt_secret_key: str = Field(default="dev-jwt-secret", validation_alias=AliasChoices("JWT_SECRET_KEY", "API_SECRET_KEY"))
    password_encryption_key: str = Field(default="dev-password-key-32-bytes-minimum", alias="PASSWORD_ENCRYPTION_KEY")
    api_token_expire_minutes: int = Field(default=480, alias="API_TOKEN_EXPIRE_MINUTES")
    ...
    opc_connect_timeout_seconds: float = Field(default=10, validation_alias=AliasChoices("OPC_CONNECT_TIMEOUT_SECONDS", "COLLECTOR_OPC_CONNECT_TIMEOUT_SECONDS"))
    opc_browse_timeout_seconds: float = Field(default=30, validation_alias=AliasChoices("OPC_BROWSE_TIMEOUT_SECONDS", "COLLECTOR_OPC_BROWSE_TIMEOUT_SECONDS"))
    opc_read_timeout_seconds: float = Field(default=10, validation_alias=AliasChoices("OPC_READ_TIMEOUT_SECONDS", "COLLECTOR_OPC_READ_TIMEOUT_SECONDS"))
    opc_browse_max_depth: int = Field(default=6, alias="OPC_BROWSE_MAX_DEPTH")
    opc_browse_max_nodes: int = Field(default=5000, alias="OPC_BROWSE_MAX_NODES")
    ...
    frontend_api_base_url: str = Field(default="http://localhost:8000", alias="FRONTEND_API_BASE_URL")
    ...
    @property
    def sqlalchemy_url_for_alembic(self) -> str:
    ...
    def mysql_connection_kwargs(self) -> dict[str, object]:
        if self.database_url:
            ...
            kwargs: dict[str, object] = {
                "host": url.host or self.db_host,
                "port": int(url.port or self.db_port),
                "user": url.username or self.db_user,
                "password": url.password or self.db_password,
                "database": (url.database or "").lstrip("/") or self.db_name,
                "connection_timeout": 10,
                "ssl_disabled": self.db_ssl_disabled,
            }
            if "ssl_ca" in query:
                kwargs["ssl_ca"] = self._resolved_ssl_ca()
            elif not self.db_ssl_disabled:
                kwargs["ssl_ca"] = self._resolved_ssl_ca()
            return kwargs
        kwargs = {
            "host": self.db_host,
